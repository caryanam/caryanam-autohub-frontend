              ...
              {fetching ? (
                Array.from({ length: 5 }).map((_, idx) => (
                  <TableRow key={`skeleton-${idx}`} className="border-b border-slate-100/80 last:border-none">
                    ...
                    <TableCell className="py-4"><Skeleton className="h-8 w-28 rounded-full" /></TableCell>
                ...
                filteredLeads.map((l, idx) => (
                  <TableRow key={l.id} className="hover:bg-slate-100 transition-colors border-b border-slate-200 last:border-none">
                    <TableCell className="text-center text-slate-400 text-sm font-medium py-4">{idx + 1}</TableCell>
                    <TableCell className="font-semibold text-slate-900 text-left text-sm py-4">{l.customerName}</TableCell>
                    <TableCell className="text-sm text-slate-500 text-left py-4">{l.mobile}</TableCell>
                    <TableCell className="text-sm font-medium text-slate-600 truncate max-w-[240px] text-left py-4">{l.vehicleTitle}</TableCell>
                    <TableCell className="text-sm text-slate-400 text-left py-4">{formatDate(l.createdAt)}</TableCell>
                    <TableCell className="text-left py-4">
                      ...
                      >
                        <SelectTrigger className={`h-8 w-[120px] text-[11px] font-bold border-0 rounded-full px-3 py-1 shadow-sm shrink-0 cursor-pointer ${COLORS[l.status]}`}>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="New">
                            <span className="text-blue-600 font-semibold text-xs">New</span>
                          </SelectItem>
                          <SelectItem value="Contacted">
                            <span className="text-amber-600 font-semibold text-xs">Contacted</span>
                          </SelectItem>
                          <SelectItem value="Converted">
                            <span className="text-emerald-600 font-semibold text-xs">Converted</span>
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </TableCell>
                    <TableCell className="text-right py-4 pr-6">
                      <div className="flex gap-1.5 justify-end">
                        <a href={`tel:${l.mobile}`}>
                          <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg bg-slate-100 text-slate-400 hover:bg-slate-300 hover:text-slate-700 transition-colors cursor-pointer">
                        ...
                        <a
                          ...
                          rel="noreferrer"
                        >
                          <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg bg-emerald-100 text-emerald-400 hover:bg-emerald-300 hover:text-emerald-600 transition-colors cursor-pointer">
