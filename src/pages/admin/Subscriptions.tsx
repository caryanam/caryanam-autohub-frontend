...
import { useAdminSubscriptions, useApprovePayment } from "@/hooks/admin/useAdminSubscriptions";
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-base text-slate-500 mt-1">{filteredSubs.length} dealer subscriptions</p>
        </div>
          ...
          <Table className="min-w-[900px]">
            <TableHeader className="bg-blue-900 border-b border-blue-900">
              <TableRow className="bg-blue-900 hover:bg-blue-900 border-none">
                ...
                <TableHead className="text-xs font-bold text-slate-100 uppercase tracking-wider py-4 pl-4">Dealer</TableHead>
                ...
                <TableHead className="text-xs font-bold text-slate-100 uppercase tracking-wider py-4">Payment ID</TableHead>
                <TableHead className="text-xs font-bold text-slate-100 uppercase tracking-wider py-4">Status</TableHead>
                <TableHead className="text-xs font-bold text-slate-100 uppercase tracking-wider py-4 pr-6 text-right">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {subLoading ? (
                Array.from({ length: 5 }).map((_, idx) => (
                  <TableRow key={`skeleton-sub-${idx}`} className="border-b border-slate-100/80 last:border-none">
                    ...
                    <TableCell className="py-4 pl-4"><Skeleton className="h-4 w-32" /></TableCell>
                    ...
                    <TableCell className="py-4 pr-6 text-right"><Skeleton className="h-8 w-24 ml-auto rounded-lg" /></TableCell>
                  </TableRow>
                ))
                ...
                <TableRow key={s.dealerId} className="hover:bg-slate-50 transition-colors border-b border-slate-200 last:border-none">
                  ...
                  <TableCell className="font-semibold text-slate-900 text-sm py-4 pl-4">{s.dealerName}</TableCell>
