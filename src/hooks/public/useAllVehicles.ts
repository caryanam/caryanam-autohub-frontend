...
export function useAllVehicles(page: number = 0, size: number = 7) {
  const { data, isLoading, isError, error, refetch, isRefetching } = useQuery<SpringPageResponse>({
    queryKey: ["all-vehicles", page, size],
    queryFn: async () => {
      const res = await fetch(`${API_BASE_URL}/api/vehicle/all-vehicle?page=${page}&size=${size}`);
