...
export default function DealerBroadcastManagement() {
  ...
  const { data: deliverySummary, isLoading: isLogsLoading, refetch: refetchLogs } =
    useOfferDeliverySummary(selectedOfferId);
  ...
  const handleSendOffer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || error.message || "Failed to send offer."
      );
    }
  };
  ...
  if (isError) {
    return (
      <div className="flex h-[40vh] flex-col items-center justify-center gap-4">
        <p className="text-muted-foreground">Failed to load dealer offers.</p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
          <h3 className="text-xl font-bold tracking-tight text-slate-950 flex items-center gap-2">
            <Gift className="h-5 w-5 text-rose-950" />
            WhatsApp Dealer Broadcasts
          </h3>
          <p className="text-muted-foreground text-sm mt-1">
            Send targeted marketing offers to all registered dealers via WhatsApp.
          </p>
        </div>
        <Button
          onClick={() => setIsSendModalOpen(true)}
          className="gap-2 bg-rose-950 hover:bg-rose-900 text-white rounded-xl shadow-md px-6"
        >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
          <div className="bg-white border border-slate-100 rounded-2xl p-5 flex flex-col items-center justify-center shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-3xl font-black text-slate-800">
              {globalStats.totalSent ?? globalStats.sent ?? 0}
            </p>
          </div>
          <div className="bg-amber-50/50 border border-amber-100 rounded-2xl p-5 flex flex-col items-center justify-center shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-3xl font-black text-amber-700">
              {globalStats.totalQueued ??
                globalStats.queued ??
                globalStats.totalAccepted ??
          ...
          <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-5 flex flex-col items-center justify-center shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-3xl font-black text-emerald-700">
              {globalStats.totalDelivered ?? globalStats.delivered ?? 0}
            </p>
          </div>
          <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-5 flex flex-col items-center justify-center shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-3xl font-black text-blue-700">
              {globalStats.totalRead ?? globalStats.read ?? 0}
            </p>
          </div>
          <div className="bg-rose-50/50 border border-rose-100 rounded-2xl p-5 flex flex-col items-center justify-center shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-3xl font-black text-rose-700">
              {globalStats.totalFailed ?? globalStats.failed ?? 0}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6">
        {offers.length === 0 ? (
          <div className="py-16 flex flex-col items-center justify-center bg-white rounded-2xl border border-dashed border-slate-300">
            ...
            <h3 className="text-lg font-semibold text-slate-800">No broadcasts sent yet</h3>
            <p className="text-slate-500 text-sm mt-1">
              Click &quot;Broadcast New Offer&quot; to reach your dealers.
            </p>
          </div>
        ) : (
          offers.map((offer) => (
            <div
              key={offer.offerId}
              className="bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col md:flex-row group"
            >
              {/* Left Side: Media & Title */}
              <div
                className="relative md:w-2/5 min-h-[280px] flex flex-col justify-between bg-slate-950 p-6 overflow-hidden"
                style={
                  ...
                  <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full mb-6">
                    ...
                    <div className="flex-1 bg-rose-50 border border-rose-100 rounded-2xl p-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-black text-rose-700 leading-none mt-1">
                          {offer.totalSentFailed}
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl flex items-center gap-2 text-slate-950">
              <Gift className="h-5 w-5 text-rose-950" /> Send Broadcast Offer to Dealers
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSendOffer} className="space-y-5 mt-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div
                  className={`border-2 rounded-xl p-4 flex items-center justify-center gap-2 cursor-pointer transition-all ${
                    templateType === "IMAGE"
                      ? "border-rose-950 bg-rose-50 text-rose-950 font-semibold"
            ...
            <div className="space-y-2">
              <Label htmlFor="broadcastOfferTitle">
                Offer Title <span className="text-red-500">*</span>
              </Label>
              <Input
                id="broadcastOfferTitle"
                value={offerTitle}
            ...
            <div className="space-y-2">
              <Label htmlFor="broadcastOfferDetails">
                Offer Details <span className="text-red-500">*</span>
              </Label>
              <Textarea
                id="broadcastOfferDetails"
                value={offerDetails}
            ...
            <div className="space-y-2">
              <Label htmlFor="broadcastBenefits">
                Benefits <span className="text-red-500">*</span>
              </Label>
              <Textarea
                id="broadcastBenefits"
                value={benefits}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="broadcastContactInfo">
                  Contact Info <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="broadcastContactInfo"
                  value={contactInfo}
              ...
              <div className="space-y-2 col-span-1 md:col-span-2">
                <Label className="text-slate-700 font-medium">
                  {templateType === "IMAGE"
                    ? "Offer Image (.jpg or .png)"
                    : "Offer Video (.mp4 or .3gp)"}{" "}
                    ...
                    {offerImage ? (
                        ...
                        <div className="flex items-center justify-between w-full px-2">
                          <p className="text-sm text-emerald-700 font-medium truncate flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-md shadow-sm border border-emerald-100">
                            ...
                            <span className="text-emerald-500 text-xs ml-1">
                              ({(offerImage.size / 1024 / 1024).toFixed(2)} MB)
                            </span>
                          </p>
                          <Button
                            type="button"
                      ...
                      <div className="flex flex-col items-center text-center p-4">
                        ...
                        <p className="text-slate-400 text-xs max-w-xs">
                          High resolution JPG or PNG up to 5MB. This will be sent directly to dealers.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    ...
                    {offerVideo ? (
                      <div className="flex flex-col items-center z-10 w-full">
                        ...
                        <div className="flex items-center justify-between w-full px-2">
                          <p className="text-sm text-emerald-700 font-medium truncate flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-md shadow-sm border border-emerald-100">
                            ...
                            {offerVideo.name}{" "}
                            <span className="text-emerald-500 text-xs ml-1">
                              ({(offerVideo.size / 1024 / 1024).toFixed(2)} MB)
                            </span>
                          </p>
                          <Button
                            type="button"
                      ...
                      <div className="flex flex-col items-center text-center p-4">
                        ...
                        <p className="text-slate-700 font-semibold text-sm mb-1">
                          Click to upload offer video
                        </p>
                        <p className="text-slate-400 text-xs max-w-xs">
                          MP4 or 3GP up to 16MB. This will be sent directly to dealers.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            <div className="pt-4 border-t flex justify-end gap-2">
              ...
              <Button
                type="submit"
                className="bg-rose-950 hover:bg-rose-900 text-white shadow-md"
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
          <DialogHeader className="pb-4 border-b">
            <div className="flex items-center justify-between pr-8">
              ...
              >
                <Loader2
                  className={`h-3.5 w-3.5 ${isLogsLoading ? "animate-spin" : ""}`}
                />
                Refresh Logs
              </Button>
            </div>
          ...
          <div className="overflow-y-auto flex-1 mt-4 pr-2">
            {isLogsLoading ? (
                ...
                <div className="space-y-3">
                  ...
                  {selectedLogs && selectedLogs.length > 0 ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-2 flex justify-between items-center text-xs text-slate-400">
                            <span>
                              Attempted: {new Date(log.sentAt).toLocaleString()}
                            </span>
                            {log.whatsappMessageId && (
                              <span
                                className="truncate max-w-[200px]"
                                title={log.whatsappMessageId}
                              >
                    ...
                    <p className="text-center text-muted-foreground py-4">
                      No specific dealer logs available.
                    </p>
                  )}
                </div>
              </div>
              ...
              <p className="text-center text-muted-foreground py-8">
                No logs available for this offer.
              </p>
            )}
          </div>
        </DialogContent>
